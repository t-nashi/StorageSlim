...
- 出力先に処理できない寸法・容量の入力を検出した場合は入力一覧に警告を表示
...
- アニメーション GIF はフレームをキャンバスへ合成してからリサイズする。GIF のフレームは部分矩形で格納されうるため、フレーム単体をリサイズすると位置とサイズが破綻するため。
...
- アニメーション GIF は全フレームを合成済みの全面フレームとして書き出すため、部分矩形による差分最適化が効かず、出力サイズが元より大きくなる場合がある。

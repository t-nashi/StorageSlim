...
- メタデータ: 削除 / 撮影日のみ / 保持（撮影日のみは GPS を落として撮影日時と向きだけを残す）
...
| PSD | 制限付き | 不可 | 統合後の画像のみ読込。RGB / 8bit のみ対応。オリジナル維持は選べない |
...
- 頒布中のビルドはタグ `v0.1.0` 時点のものです。この README に載っている機能がすべて含まれているとは限りません。最新の機能を試す場合は master をビルドしてください。
...
- メタデータの引き継ぎは EXIF のみが対象。XMP と ICC プロファイルは引き継がない。
- EXIF を引き継げるのは JPEG / PNG / WebP 出力。AVIF / GIF 出力と HEIC / HEIF 入力では引き継げず、警告を表示する。
- `撮影日のみ` は撮影日時と向きのタグだけを残して EXIF を組み直す。GPS とメーカーノートは構造ごと落ちる。クラウドの写真サービスへ上げ直したときに撮影日で並ぶことと、位置情報を残さないことを両立させるための選択肢。
- リサイズした場合、EXIF が持つ画素数のタグは出力の実寸へ書き換える。
